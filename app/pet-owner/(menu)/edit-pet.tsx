...
const EditPet = () => {
  ...
  const [profileImage, setProfileImage] = useState<string | null>(img_path);
  const [imageChanged, setImageChanged] = useState(false);
  const [species, setSpecies] = useState(_species);
  const [name, setName] = useState(_name);
  const [breed, setBreed] = useState(_breed);
  const [age, setAge] = useState(_age);
  const [gender, setGender] = useState<"Male" | "Female" | "">(_gender);
  ...
  useOnFocusHook(() => {
    ...
    const parsedVaccines = JSON.parse(_vaccines);
    ...
    if (!name || !species || !breed || !gender || !profileImage) throw "Please fill all fields!!!"
      ...
      if (profileImage) {
        if (imageChanged) {
          ...
          data.img_path = profileImage;
        }
      }

      await setUnMerged("users", userId, "pets", id).value({
        name: name,
        species: species,
        breed: breed,
        age: age,
